...
for ch in table_328_channels:
    ch_str = f"PK {int(ch['start_pk']//1000)}+{int(ch['start_pk']%1000):03d} – PK {int(ch['end_pk']//1000)}+{int(ch['end_pk']%1000):03d}"
...
# -------------------------------------------------------------
# 5. LONGITUDINAL DITCHES FROM PLAN SHEETS (DW-03001 to DW-03047)
# -------------------------------------------------------------
print("\n--- 5. Longitudinal Ditches from Plan Sheets ---")
...
for d in plan_sheets:
    ...
    # Fit station ticks for Strip 1 (y: 700-780) and Strip 2 (y: 1480-1560)
    s1_ticks = []
    s2_ticks = []
    for w in words:
        ...
        if m:
            ...
            if 700 <= w[1] <= 780:
                s1_ticks.append((w[0], pk))
            elif 1480 <= w[1] <= 1560:
                s2_ticks.append((w[0], pk))
...
print(f"Extracted {len(raw_ditch_runs)} raw ditch runs across all plan sheets.")

# Now stitch adjacent/overlapping runs of the same type and side
...
for run in raw_ditch_runs:
    ...
    # Check if same type and side, and start_pk is close to or overlaps last.end_pk (within 25m)
    if last["type"] == run["type"] and last["side"] == run["side"] and (run["start_pk"] <= last["end_pk"] + 25.0):
        ...
        # If the newer run has higher revision or different sheet, record both
...
print(f"Stitched into {len(stitched_runs)} continuous longitudinal ditch features.")

for r in stitched_runs:
    spk = round(r["start_pk"], 1)
    epk = round(r["end_pk"], 1)
    length = round(epk - spk, 1)
    if length < 5.0: # Skip micro-segments under 5m
        continue
...
# -------------------------------------------------------------
# 6. TEXT CALLOUT FEATURES (Berm Ditches & Riprap Protection)
# -------------------------------------------------------------
print("\n--- 6. Text Callouts (Berm Ditches & Riprap Protection) ---")

callouts_added = 0
for d in plan_sheets:
    pdf_fn = d.get("pdf_filename")
    if not pdf_fn: continue
    pdf_path = os.path.join(DRAWINGS_DIR, pdf_fn)
    ...
    for b in blocks:
        ...
        if m_hr:
            ...
            # Extract stated length if present
...
print(f"Added {callouts_added} text callout features.")
